...
AutoHealthKey        := "F1" ; Automatic snacking. Eats 4 snacks from second snack slot.
...
TogglePassiveKey     := "F6" ; Toggle passive mode.
EquipScarfKey        := "NumpadDot" ; Equip first scarf (heist outfit glitch, see readme/misc).
EWOKey       		 := "F5" ; Toggle between extended and standar radar.
ToggleRadarKey       := "+F5" ; Toggle between extended and standar radar.
CycleOutfitKey       := "Numpad0" ; Equip next/cycle through saved outfits.
...
ToggleClickerKey     := "X" ; Toggle Clicker (XButton2 = Mouse5)
...
RandomHeistKey       := "F10" ; Chooses on-call random heist from phone options
CEOBuzzardKey        := "^F9" ; Spawn free CEO buzzard

DialDialogKey        := "F7" ; Call GUI with a list of almost all numbers
CallMechanicKey      := "+F3" ; Call Mechanic
CallPegasusKey       := "^F3" ; Call Pegasus
CallMerryweatherKey  := "F9" ; Call Merryweather
CallInsuranceKey     := "<!F3" ; Call Insurance
CallLesterKey        := "F4" ; Call Lester
CallAssistantKey     := "+F9" ; Call Assistant

CheckForUpdatesKey   := "F24" ; Checks on startup by default, see DoCheckForUpdates option


ReloadMacroKey		 := "NumpadAdd" ; Reloads this macro script
SuspendMacroKey		 := "+NumpadAdd" ; Suspends this macro script
...
ArrayChatSnippets := ["ублюдок мать твою, а ну иди сюда говно собачье, решил ко мне лезть? ты засранец вонючий, мать твою, а? Ну иди сюда, попробуй меня трахнуть"]
ArrayChatSnippets.push("я тебя сам трахну ублюдок, онанист чертов, БУДЬ ТЫ ПРОКЛЯТ, ИДИ ИДИОТ ТРАХАТЬ ТЕБЯ И ВСЮ СЕМЬЮ")
ArrayChatSnippets.push("ГОВНО СОБАЧЬЕ ЖЛОБ ВОНЮЧИЙ, дерьмо, сука, падла, иди сюда, мерзавец, негодяй, гад иди сюда ТЫ ГОВНО, ЖОПА")
ArrayChatSnippets.push("Давай, мочи их")
ArrayChatSnippets.push("Ща порвем его")
ArrayChatSnippets.push("Ну все, блять,ты огребаешь")
ArrayChatSnippets.push("Ах ты ебоглаз")


; Delays (you normally don't want to change these, you can try to play with these values if you have a slow/fast PC)
IntFocusDelay        := 90  ; delay (in ms) after focussing game when AHK-GUI took focus.
IntMenuDelay         := 110  ; delay (i	n ms) after opening interaction menu.
...
IntPhoneScrollDelay  := 70   ; delay (in ms) between scrolls in the phone menu.
IntKeySendDelay      := 20   ; delay (in ms) delay between send key commands.
...
bringGameIntoFocus()
  ...
  {
	  Click
      if !GetKeyState(%ToggleClickerKey%, "P")  ; The key has been released, so break out of the loop.
          break
  }
  return
...
AutoHealth:
  openInteractionMenu(IsVIPActivated, IsCPHActivated)
  openSnackMenu()
  Send {Down}{Enter 4}{%IGB_Interaction%}
  return
...
AutoArmor:
  openInteractionMenu(IsVIPActivated, IsCPHActivated)
  openArmorMenu()
  Send {Down 4}{Enter}{%IGB_Interaction%}
  return
...
RetrieveCar:
  openInteractionMenu(IsVIPActivated, IsCPHActivated)
  Send {Down 4}{Enter 2}{%IGB_Interaction%}
  return
...
ChatSnippets:
  turnCapslockOff()
  Gui, CSNIP:add, Text, , angry scav line:
